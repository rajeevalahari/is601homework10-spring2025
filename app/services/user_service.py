...
class UserService:
    ...
    async def create(cls, session: AsyncSession, user_data: Dict[str, str], email_service: EmailService) -> Optional[User]:
        try:
            # Validate and parse the incoming data using the UserCreate schema.
            ...
            if provided_nickname:
                if await cls.get_by_nickname(session, provided_nickname):
                    logger.error(f"Nickname '{provided_nickname}' is already in use.")
                    return None
            else:
                # Generate a unique nickname if none is provided.
                provided_nickname = generate_nickname()
                while await cls.get_by_nickname(session, provided_nickname):
                    provided_nickname = generate_nickname()
                validated_data["nickname"] = provided_nickname

            # Hash the provided password and remove it from validated data.
            validated_data['hashed_password'] = hash_password(validated_data.pop('password'))

            # Create the User instance and set additional attributes.
